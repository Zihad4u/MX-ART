...
const AddItemCard = ({ cardPass }) => {
    const [userData, setData] = useState(cardPass || []);
    const handleDelete = _id => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                //     icon: "success"
                // });
                fetch(`https://assignment-10-server-blond-one.vercel.app/myItemDelete/${_id}`, {
                    method: 'DELETE'
                })
                    .then(res => res.json())
                    .then(() => {
                        // console.log(data)
                        // const newData = userData.filter(item => item._id !== _id);
                        // setData(newData);
    ...
    const { image, item_name, rating, customization, short_description, price, _id, processing_time, stockStatus } = userData;
    useEffect(() => {
        Aos.init();
    }, [])
    ...
    return (
        <div className="hover:transform  hover:scale-105 transition-transform duration-300  hover:shadow-xl shadow-lg" >
            <Card data-aos="fade-up" className={cardClasses} >
                ...
                <div className="pt-3 w-full flex self-start mt-auto">
                    <NavLink onClick={() => handleDelete(_id)} className="btn w-[50%] text-white bg-black   " size="lg" >
                        Delete
                    </NavLink>
                    <NavLink to={`/update/${_id}`} className="btn bg-black text-white  w-[50%] " size="lg" fullWidth={true}>
                        Update
                    </NavLink>
                </div>
            </Card>
